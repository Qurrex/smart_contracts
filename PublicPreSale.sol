...
contract BsTokensale is SafeMath, Ownable, Pausable {
    ...
    function BsTokensale(address _token, address _beneficiary) Ownable() public {
        ...
        minAmountWei = 15 ether;
        tokensPerEth = 840; // Default rate
        
        maxCapInTokens = 17 * 1e6 ether - 381235 ether; // 17 m tokens.

        startTime = 1519722000; // 2018-02-27T09:00:00Z
        endTime   = 1522141200; // 2018-03-27T09:00:00Z
    }
    ...
    function weiToTokens(uint256 _weiValue) public view returns (uint256) {
        ...
        if (_weiValue >= 750 ether) {
            tokenRate = 1120;
        } else if (_weiValue >= 150 ether) {
            tokenRate = 1000;
        } else if (_weiValue >= 30 ether) {
            tokenRate = 920;
        } else if(_weiValue >= 15 ether) {
            tokenRate = 840;
        }
...
contract ProdToken is BsToken {
    ...
    function ProdToken() BsToken(
        0x788cdc97403a5CAfa60D9aAa7E8265f83D629DF5, // seller address
        'Qurrex',
        'QRX',
        70000000,
        55000000
...
contract PublicPreSale is BsTokensale {

    function PublicPreSale() BsTokensale(
        0x1d5a98162A497CD204948bc5E00E98B44ABC7Cd4, // Token address
        0xCc166a2562D0FC6cB7cbEe42Ce95B4724c4d3d75  // Beneficiary address
    ) public { }
}
